...
TfLiteStatus RecognizeCommands::ProcessLatestResults(
  ...
  if ((latest_results->dims->size != 2) ||
      ...
      (latest_results->dims->data[1] != KWSCategoryCount)) {
    TF_LITE_REPORT_ERROR(
        error_reporter_,
        "The results for recognition should contain %d elements, but there are "
        "%d in an %d-dimensional shape",
        KWSCategoryCount, latest_results->dims->data[1],
  ...
  int32_t average_scores[KWSCategoryCount];
  for (int offset = 0; offset < previous_results_.size(); ++offset) {
    PreviousResultsQueue::Result previous_result =
        previous_results_.from_front(offset);
    const int8_t* scores = previous_result.scores;
    for (int i = 0; i < KWSCategoryCount; ++i) {
      if (offset == 0) {
  ...
  const char* current_top_label = KWSCategoryLabels[current_top_index];

  // If we've recently had another label trigger, assume one that occurs too
  // soon afterwards is a bad result.
  int64_t time_since_last_top;
  if ((previous_top_label_ == KWSCategoryLabels[0]) ||
      (previous_top_label_time_ == std::numeric_limits<int32_t>::min())) {
