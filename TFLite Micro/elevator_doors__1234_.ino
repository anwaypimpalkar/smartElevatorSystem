...
void setup()
{
  ...
  //create the allocator that will be shared between the VWW and KWS models
  tflite::MicroAllocator *allocator =
    tflite::MicroAllocator::Create(tensor_arena, kTensorArenaSize,
                                   error_reporter);

  // Map the model into a usable data structure from the binary weights
  ...
  // Prepare to access the audio spectrograms from a microphone that will provide the inputs to the neural network
...
void loop()
{
  ...
  if (ifPerson)
  {
    ...
    while (millis() - start_time < 5000)
    {
      ...
      // Respond to command, show digit on 7 segment display
